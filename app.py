# -*- coding: utf-8 -*-
"""
HidroSed · Módulo Eje Cauce y Secciones v15 · geometría ajustada a extremos
...
# --------------------------------------------------------------------------------------
# Inserción local de geometría trapecial/rectangular ajustada a extremos
# --------------------------------------------------------------------------------------
def _clean_station_elevation_pairs(stations: Sequence[float], elevations: Sequence[float]) -> Tuple[List[float], List[float]]:
    """Ordena y elimina estaciones duplicadas conservando la última cota ingresada."""
    pairs = []
    for s, z in zip(stations, elevations):
        try:
            s2 = float(s)
            z2 = float(z)
        except Exception:
            continue
        if not (np.isfinite(s2) and np.isfinite(z2)):
            continue
        pairs.append((s2, z2))
    pairs.sort(key=lambda x: x[0])
    out = []
    for s, z in pairs:
        if out and abs(out[-1][0] - s) < 1e-6:
            out[-1] = (s, z)
        else:
            out.append((s, z))
...
def _section_extremes(sec: CrossSection) -> Tuple[float, float, float, float]:
    """Devuelve station/cota de extremo izquierdo y derecho de una sección."""
    st_arr = np.array(sec.stations, dtype=float)
    z_arr = np.array(sec.elevations, dtype=float)
    valid = np.isfinite(st_arr) & np.isfinite(z_arr)
    if valid.sum() < 2:
        raise ValueError("La sección no tiene al menos dos puntos station-cota válidos para identificar extremos.")
    st_arr = st_arr[valid]
    z_arr = z_arr[valid]
    i_l = int(np.argmin(st_arr))
    i_r = int(np.argmax(st_arr))
    return float(st_arr[i_l]), float(z_arr[i_l]), float(st_arr[i_r]), float(z_arr[i_r])


def build_embedded_channel_between_extremes(sec: CrossSection, axis_useful: LineString,
                                            kind: str,
                                            s_left: float, z_left: float,
                                            s_right: float, z_right: float,
                                            bottom_width: float, height: float,
                                            side_slope_l: float, side_slope_r: float,
                                            bed_elevation: float,
                                            fit_mode: str,
                                            manning_n: float,
                                            plan: str,
                                            note: str = "") -> CrossSection:
    """Inserta una geometría rectangular/trapecial dentro de dos extremos definidos.

    El objetivo es explícito: los extremos izquierdo/derecho de la sección resultante
    coinciden con los extremos definidos por el usuario o por la sección existente.
    La geometría prismática se inserta al centro o usando los extremos como coronamiento,
    según `fit_mode`.
    """
    sL, zL, sR, zR = float(s_left), float(z_left), float(s_right), float(z_right)
    if sR < sL:
        sL, sR = sR, sL
        zL, zR = zR, zL
    width = max(sR - sL, 0.001)
    b = max(float(bottom_width), 0.001)
    h = max(float(height), 0.001)
    zl = max(float(side_slope_l), 0.0)
    zr = max(float(side_slope_r), 0.0)
    zbed = float(bed_elevation)
    kind_l = str(kind).lower()
    fit_l = str(fit_mode).lower()
    warnings = []

    if kind_l.startswith("rect"):
        if "extremos" in fit_l:
            # La sección rectangular ocupa todo el ancho entre extremos.
            stations = [sL, sL, sR, sR]
            elevations = [zL, zbed, zbed, zR]
            effective = {"bottom_width_effective": width, "height_effective": max(zL, zR) - zbed}
            if abs(b - width) > max(0.01, 0.02 * width):
                warnings.append(f"Ancho de fondo ingresado {b:.2f} m ajustado al ancho entre extremos {width:.2f} m para coincidir extremos.")
        else:
            # Canal rectangular interior con transiciones laterales hacia los extremos.
            b_eff = min(b, width)
            ch_l = (sL + sR - b_eff) / 2.0
            ch_r = ch_l + b_eff
            ztop = zbed + h
            stations = [sL, ch_l, ch_l, ch_r, ch_r, sR]
            elevations = [zL, ztop, zbed, zbed, ztop, zR]
            effective = {"bottom_width_effective": b_eff, "height_effective": h}
            if b_eff < b:
                warnings.append(f"Ancho de fondo reducido de {b:.2f} a {b_eff:.2f} m para caber entre extremos.")
    else:
        if "extremos" in fit_l:
            # Los extremos definidos son los bordes superiores del trapecio.
            top_width = width
            if zl + zr > 0:
                max_h = max((top_width - 0.10) / max(zl + zr, 1e-9), 0.001)
                h_eff = min(h, max_h)
            else:
                h_eff = h
            b_eff = max(top_width - (zl + zr) * h_eff, 0.10)
            bl = sL + zl * h_eff
            br = sR - zr * h_eff
            if br <= bl:
                mid = (sL + sR) / 2.0
                ...
                br = mid + 0.05
                b_eff = 0.10
                warnings.append("Trapecio muy estrecho para los taludes/altura; se dejó fondo mínimo de control.")
            if abs(b_eff - b) > max(0.01, 0.05 * max(b, 1.0)):
                warnings.append(f"Ancho de fondo efectivo {b_eff:.2f} m difiere del ingresado {b:.2f} m para coincidir con extremos.")
            stations = [sL, bl, br, sR]
            elevations = [zL, zbed, zbed, zR]
            effective = {"bottom_width_effective": b_eff, "height_effective": h_eff}
        else:
            # Se conserva la geometría ingresada y se conecta con los extremos mediante transiciones laterales.
            top_req = b + (zl + zr) * h
            if top_req > width:
                # Ajuste conservador: primero reduce el ancho de fondo; si no basta, reduce altura.
                b_eff = max(width - (zl + zr) * h, 0.10)
                h_eff = h
                if b_eff <= 0.101 and zl + zr > 0:
                    h_eff = max((width - 0.10) / (zl + zr), 0.001)
                    b_eff = 0.10
                warnings.append("La geometría ingresada excedía el ancho entre extremos; se ajustó internamente para no sobrepasarlos.")
            else:
                b_eff = b
                h_eff = h
            top_width = b_eff + (zl + zr) * h_eff
            top_l = (sL + sR - top_width) / 2.0
            top_r = top_l + top_width
            bl = top_l + zl * h_eff
            br = bl + b_eff
            ztop = zbed + h_eff
            stations = [sL, top_l, bl, br, top_r, sR]
            elevations = [zL, ztop, zbed, zbed, ztop, zR]
            effective = {"bottom_width_effective": b_eff, "height_effective": h_eff}

    stations, elevations = _clean_station_elevation_pairs(stations, elevations)
    old_geom = {
        "stations": list(sec.stations),
        "elevations": list(sec.elevations),
        "source": sec.source,
        "section_type": sec.section_type,
    }
    sec.stations = [float(v) for v in stations]
    sec.elevations = [float(v) for v in elevations]
    sec.source = f"geometría {kind_l} insertada entre extremos"
    sec.section_type = f"{kind_l}_insertado_en_seccion"
    sec.origin_type = "modificada"
    sec.plan = plan or sec.plan
    sec.manning_lob = float(manning_n)
    sec.manning_channel = float(manning_n)
    sec.manning_rob = float(manning_n)
    sec.status = "corregida" if not warnings else "revisar"
    sec.notes = (sec.notes or "") + f" | geometría {kind_l} insertada haciendo coincidir extremos station {sL:.2f} y {sR:.2f}. {note}".strip()
    if warnings:
        sec.notes += " Advertencias: " + " ".join(warnings)

    # Actualiza línea de sección para cubrir el ancho definido por los extremos.
    try:
        d = min(max(float(sec.km) * 1000.0, 0.0), float(axis_useful.length))
        sec_line = cross_section_line(axis_useful, d, max(abs(sL), abs(sR), width / 2.0, 1.0))
        sec.left_x, sec.left_y = sec_line.coords[0]
        sec.right_x, sec.right_y = sec_line.coords[-1]
        center = axis_useful.interpolate(d)
        sec.center_x, sec.center_y = float(center.x), float(center.y)
    except Exception:
        pass

    _append_change(
        sec,
        "insertar_geometria_entre_extremos",
        "geometria",
        old_geom,
        {"stations": sec.stations, "elevations": sec.elevations, "kind": kind_l, "effective": effective, "warnings": warnings},
        warnings=warnings,
        plan=plan,
    )
    return qa_section(sec, axis_useful=axis_useful)
...
def apply_embedded_channel_between_extremes_by_km(sections: List[CrossSection], axis_useful: LineString,
                                                  km_from: float, km_to: float,
                                                  kind: str,
                                                  use_existing_extremes: bool,
                                                  manual_s_left: float, manual_z_left: float,
                                                  manual_s_right: float, manual_z_right: float,
                                                  bottom_width: float, height: float,
                                                  side_slope_l: float, side_slope_r: float,
                                                  bed_mode: str, bed_elevation_manual: float,
                                                  fit_mode: str,
                                                  manning_n: float,
                                                  plan: str,
                                                  note: str = "") -> Tuple[List[CrossSection], int, int]:
    ...
    for i, sec in enumerate(sections):
        if not (a <= float(sec.km) <= b):
            continue
        try:
            if use_existing_extremes:
                sL, zL, sR, zR = _section_extremes(sec)
            else:
                sL, zL, sR, zR = float(manual_s_left), float(manual_z_left), float(manual_s_right), float(manual_z_right)
            if str(bed_mode).lower().startswith("manual"):
                zbed = float(bed_elevation_manual)
            else:
                zbed = min(float(zL), float(zR)) - float(height)
            sections[i] = build_embedded_channel_between_extremes(
                sec, axis_useful, kind, sL, zL, sR, zR,
                bottom_width, height, side_slope_l, side_slope_r,
                zbed, fit_mode, manning_n, plan, note,
            )
            count += 1
        except Exception as exc:
            sec.status = "revisar"
            sec.notes = (sec.notes or "") + f" | no se pudo insertar geometría entre extremos: {exc}"
            sections[i] = sec
            fail += 1
    return sections, count, fail
...
"""
            ...
            with st.expander("Insertar geometría trapecial/rectangular ajustada a extremos de sección", expanded=False):
                st.markdown(
                    """
Esta herramienta es para modificar **parte de la sección existente**, no para reemplazarla completa.
Primero se identifican los dos extremos de la sección —izquierdo y derecho, mirando aguas abajo— y luego
se inserta una geometría rectangular o trapecial que queda conectada a esos extremos.

Uso típico: definir entre qué km se aplica, elegir los extremos existentes o ingresarlos manualmente,
indicar ancho de fondo, altura, taludes y cota de fondo. La aplicación ajusta la geometría para que
los extremos de la sección resultante coincidan con los extremos definidos.
"""
                )
                eg1, eg2, eg3, eg4 = st.columns(4)
                eg_km_from = eg1.number_input("Desde km", min_value=min_km, max_value=max_km, value=km_from, step=0.001, format="%.3f", key="eg_km_from")
                eg_km_to = eg2.number_input("Hasta km", min_value=min_km, max_value=max_km, value=km_to, step=0.001, format="%.3f", key="eg_km_to")
                eg_kind = eg3.selectbox("Tipo geometría", ["trapecial", "rectangular"], index=0, key="eg_kind")
                eg_n = eg4.number_input("Manning n", min_value=0.010, max_value=0.200, value=float(result.get("manning_n", 0.035)), step=0.001, format="%.3f", key="eg_n")

                eg_source = st.radio(
                    "Extremos de la sección",
                    ["Usar extremos existentes de cada sección", "Ingresar extremos manuales"],
                    index=0,
                    horizontal=True,
                    key="eg_source",
                )
                use_existing_ext = eg_source.startswith("Usar")
                if use_existing_ext:
                    st.caption("La app toma el station mínimo y máximo de cada sección como extremos y conserva esos puntos como anclajes.")
                    man_s_left = man_z_left = man_s_right = man_z_right = 0.0
                else:
                    st.caption("Ingresa los dos extremos que deben mantenerse al insertar la nueva geometría.")
                    mx1, mx2, mx3, mx4 = st.columns(4)
                    man_s_left = mx1.number_input("Station extremo izquierdo [m]", value=float(min(sec.stations) if sec.stations else 0.0), step=0.5, key="eg_s_left")
                    man_z_left = mx2.number_input("Cota extremo izquierdo [m]", value=float(sec.elevations[0] if sec.elevations else 0.0), step=0.05, key="eg_z_left")
                    man_s_right = mx3.number_input("Station extremo derecho [m]", value=float(max(sec.stations) if sec.stations else 20.0), step=0.5, key="eg_s_right")
                    man_z_right = mx4.number_input("Cota extremo derecho [m]", value=float(sec.elevations[-1] if sec.elevations else 0.0), step=0.05, key="eg_z_right")

                gp1, gp2, gp3, gp4 = st.columns(4)
                eg_b = gp1.number_input("Ancho de fondo [m]", min_value=0.10, max_value=2000.0, value=float(result.get("bottom_width", 5.0)), step=0.5, key="eg_b")
                eg_h = gp2.number_input("Altura / profundidad [m]", min_value=0.10, max_value=100.0, value=float(result.get("height", 2.0)), step=0.1, key="eg_h")
                eg_zl = gp3.number_input("Talud izquierdo H:V", min_value=0.0, max_value=50.0, value=float(result.get("talud_l", 1.5)), step=0.1, key="eg_zl")
                eg_zr = gp4.number_input("Talud derecho H:V", min_value=0.0, max_value=50.0, value=float(result.get("talud_r", 1.5)), step=0.1, key="eg_zr")

                ez1, ez2, ez3 = st.columns(3)
                eg_bed_mode = ez1.selectbox("Cota de fondo", ["Automática: mínimo extremo - altura", "Manual"], index=0, key="eg_bed_mode")
                eg_bed_manual = ez2.number_input("Cota fondo manual [m]", value=float(np.nanmin(sec.elevations) if sec.elevations else 0.0), step=0.05, key="eg_bed_manual", disabled=not eg_bed_mode.startswith("Manual"))
                eg_fit = ez3.selectbox(
                    "Modo de coincidencia",
                    ["Canal interior con transiciones a extremos", "Extremos son bordes superiores del canal"],
                    index=0,
                    key="eg_fit",
                )
                eg_plan = st.text_input("Plan / alternativa", value="Modificado", key="eg_plan")
                eg_note = st.text_input("Observación", value="geometría insertada ajustada a extremos", key="eg_note")

                st.info(
                    "Recomendación: usa 'Canal interior con transiciones a extremos' cuando quieres conservar el terreno natural lateral y encajar un canal dentro de la sección. "
                    "Usa 'Extremos son bordes superiores' cuando los dos extremos ingresados corresponden directamente a los bordes superiores del canal."
                )
                if st.button("Insertar geometría ajustada a extremos", key="eg_apply"):
                    sections, neg, nfail = apply_embedded_channel_between_extremes_by_km(
                        sections, axis_useful, eg_km_from, eg_km_to, eg_kind,
                        use_existing_ext, man_s_left, man_z_left, man_s_right, man_z_right,
                        eg_b, eg_h, eg_zl, eg_zr,
                        eg_bed_mode, eg_bed_manual, eg_fit,
                        eg_n, eg_plan, eg_note,
                    )
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.success(f"Se insertó geometría {eg_kind} en {neg} secciones. Fallidas: {nfail}.")
                    st.rerun()
...
# ======================================================================================
# HIDROSED_V1 · Módulo Cuenca y Morfometría
# Este módulo aporta exclusivamente propiedades morfométricas de cuenca, con base técnica
# exportable para hidrología/hidráulica/sedimentos. No reemplaza el módulo v15 de secciones.
# ======================================================================================

def _m_uploaded_to_bytesupload(name: str, data: bytes):
    return BytesUpload(name, data)


def _m_read_dem_as_projected_array(uploaded_file, crs_proj: CRS, target_resolution_m: float = 60.0):
    """Lee un GeoTIFF y lo reproyecta/remuestrea a CRS proyectado local."""
    from rasterio.io import MemoryFile
    from rasterio.warp import calculate_default_transform, reproject, Resampling
    from affine import Affine
    data = uploaded_bytes(uploaded_file)
    with MemoryFile(data) as mem:
        with mem.open() as src_ds:
            if src_ds.crs is None:
                raise RuntimeError("El DEM no tiene CRS definido.")
            dst_transform, dst_width, dst_height = calculate_default_transform(
                src_ds.crs, crs_proj, src_ds.width, src_ds.height, *src_ds.bounds,
                resolution=float(target_resolution_m)
            )
            arr = np.empty((dst_height, dst_width), dtype="float32")
    ...
    return arr, dst_transform, valid


def _m_world_to_rowcol(transform, x: float, y: float):
    inv = ~transform
    c, r = inv * (x, y)
    return int(round(r)), int(round(c))


def _m_rowcol_to_world(transform, r: int, c: int):
    x, y = transform * (c + 0.5, r + 0.5)
    return float(x), float(y)


def _m_flow_d8(dem: np.ndarray, valid: np.ndarray):
    """Dirección de flujo D8 simple: cada celda apunta al vecino de menor cota."""
    h, w = dem.shape
    rec = np.full(h * w, -1, dtype=np.int64)
    # vecinos: fila, col, distancia relativa
    neigh = [(-1,-1,math.sqrt(2)),(-1,0,1),(-1,1,math.sqrt(2)),(0,-1,1),(0,1,1),(1,-1,math.sqrt(2)),(1,0,1),(1,1,math.sqrt(2))]
    for r in range(h):
        for c in range(w):
            if not valid[r,c]:
                continue
            ...
            for dr, dc, dist in neigh:
                ...
                if 0 <= rr < h and 0 <= cc < w and valid[rr,cc]:
                    dz = z - float(dem[rr,cc])
                    if dz > 0:
                        s = dz / dist
                        if best is None or s > best[0]:
                            best = (s, rr, cc)
            if best is not None:
                rec[r*w+c] = best[1]*w + best[2]
    return rec


def _m_flow_accumulation(rec: np.ndarray, valid_flat: np.ndarray):
    n = rec.size
    ...
    q = deque([i for i in range(n) if valid_flat[i] and indeg[i] == 0])
    while q:
        i = q.popleft()
        j = rec[i]
        if j >= 0:
...
def _m_snap_to_drainage(point: Point, acc2d: np.ndarray, valid: np.ndarray, transform, radius_m: float, axis: Optional[LineString], axis_buffer_m: float):
    h, w = acc2d.shape
    r0, c0 = _m_world_to_rowcol(transform, point.x, point.y)
    px = max(abs(float(transform.a)), abs(float(transform.e)), 1.0)
    rad = max(1, int(math.ceil(float(radius_m) / px)))
    ...
    for r in range(max(0, r0-rad), min(h, r0+rad+1)):
        for c in range(max(0, c0-rad), min(w, c0+rad+1)):
            if not valid[r,c]:
                continue
            x, y = _m_rowcol_to_world(transform, r, c)
            d = math.hypot(x-point.x, y-point.y)
            if d > radius_m:
                continue
            if axis_buf is not None and not axis_buf.contains(Point(x,y)):
                continue
            score = (float(acc2d[r,c]), -d)
            if best is None or score > best[0]:
                best = (score, r, c, d, float(acc2d[r,c]))
    if best is None:
        raise RuntimeError("No se pudo ajustar el punto al drenaje. Aumente radio de ajuste, revise DEM o eje.")
    _, r, c, d, a = best
    return int(r), int(c), float(d), float(a)


def _m_upstream_mask(rec: np.ndarray, valid_flat: np.ndarray, outlet_idx: int):
    children = [[] for _ in range(rec.size)]
    for i, j in enumerate(rec):
        if valid_flat[i] and j >= 0:
            children[j].append(i)
...
def _m_polygonize_mask(mask2d: np.ndarray, transform):
...
def _m_mask_touches_edge(mask2d: np.ndarray):
    return bool(mask2d[0,:].any() or mask2d[-1,:].any() or mask2d[:,0].any() or mask2d[:,-1].any())


def _m_delineate_single_basin(pc: Point, name: str, dem: np.ndarray, transform, valid: np.ndarray, rec: np.ndarray, acc: np.ndarray, snap_radii: List[float], axis: Optional[LineString], axis_buffer_m: float):
    acc2d = acc.reshape(dem.shape)
    valid_flat = valid.ravel()
    best = None
    for radius in snap_radii:
        try:
            r, c, dist, acc_cells = _m_snap_to_drainage(pc, acc2d, valid, transform, radius, axis, axis_buffer_m)
            idx = r * dem.shape[1] + c
            mask = _m_upstream_mask(rec, valid_flat, idx).reshape(dem.shape)
            poly = _m_polygonize_mask(mask, transform)
            area_km2 = poly.area / 1e6
            if not np.isfinite(area_km2) or area_km2 <= 0:
                continue
            touches = _m_mask_touches_edge(mask)
            shape_idx = poly.length / max(2 * math.sqrt(math.pi * max(poly.area, 1.0)), 1e-9)
            score = 100.0
            if touches: score -= 30
            if dist > 0.8 * radius: score -= 10
            if shape_idx > 4.5: score -= 10
            if area_km2 < 0.01: score -= 30
            if axis is not None:
                frac = axis.intersection(poly).length / max(axis.length, 1.0)
                if frac < 0.30:
                    score -= 15
            cand = {
                "name": name, "polygon": poly, "mask": mask, "snap_distance_m": dist,
                "snap_radius_m": radius, "acc_cells": acc_cells, "touches_edge": touches,
                "confidence_pct": float(max(0, min(100, score))), "shape_index": float(shape_idx),
                "snapped_x": _m_rowcol_to_world(transform, r, c)[0], "snapped_y": _m_rowcol_to_world(transform, r, c)[1],
            }
...
def _m_values_in_geom(dem: np.ndarray, transform, valid: np.ndarray, geom):
    from rasterio import features
...
def _m_slope_mean_pct(dem: np.ndarray, transform, valid: np.ndarray, geom):
    from rasterio import features
    arr = dem.astype(float)
    dx = max(abs(float(transform.a)), 1.0)
    dy = max(abs(float(transform.e)), 1.0)
...
def _m_morphometry(name: str, basin: Dict[str, Any], dem: np.ndarray, transform, valid: np.ndarray, crs_proj: CRS):
    poly = basin["polygon"]
    vals = _m_values_in_geom(dem, transform, valid, poly)
    area_km2 = float(poly.area / 1e6)
    perim_km = float(poly.length / 1000)
    minx, miny, maxx, maxy = poly.bounds
    bbox_len_km = float(max(maxx-minx, maxy-miny) / 1000)
    kc = perim_km / max(2 * math.sqrt(math.pi * area_km2), 1e-9)
    eqd = 2 * math.sqrt(area_km2 / math.pi) if area_km2 > 0 else float("nan")
    ff = area_km2 / max(bbox_len_km * bbox_len_km, 1e-9)
    elong = eqd / max(bbox_len_km, 1e-9)
    ...
    cent = poly.centroid
    warnings = []
    if basin["touches_edge"]:
        warnings.append("La cuenca toca el borde del DEM; ampliar DEM antes de usar como definitiva.")
    if basin["confidence_pct"] < 80:
        warnings.append("Confianza geométrica media/baja; revisar punto de control, eje y DEM.")
    return {
        "nombre": name,
        "area_km2": area_km2,
        "area_ha": area_km2*100,
        "perimetro_km": perim_km,
        "coef_compacidad_kc": float(kc),
        "diametro_equivalente_km": float(eqd),
        "longitud_bbox_km": bbox_len_km,
        "factor_forma": float(ff),
        "relacion_elongacion": float(elong),
        "cota_min_m": zmin,
        "cota_media_m": zmean,
        "cota_max_m": zmax,
        "desnivel_m": float(zmax-zmin) if np.isfinite(zmax-zmin) else float("nan"),
        "pendiente_media_pct": _m_slope_mean_pct(dem, transform, valid, poly),
        "centroide_x": float(cent.x), "centroide_y": float(cent.y),
        "toca_borde_dem": bool(basin["touches_edge"]),
        "confianza_pct": float(basin["confidence_pct"]),
        "snap_distancia_m": float(basin["snap_distance_m"]),
        "snap_radio_m": float(basin["snap_radius_m"]),
        "acumulacion_celdas": float(basin["acc_cells"]),
        "advertencias": "; ".join(warnings),
    }


def _m_hypsometric_curve(dem: np.ndarray, transform, valid: np.ndarray, geom, bins: int = 40):
    vals = _m_values_in_geom(dem, transform, valid, geom)
    if vals.size < 5:
        return pd.DataFrame(columns=["elevacion_m", "area_relativa_pct"])
    vals = np.sort(vals)
    elev = np.linspace(float(vals.min()), float(vals.max()), int(bins))
    frac = [(vals >= z).sum() / vals.size * 100 for z in elev]
    return pd.DataFrame({"elevacion_m": elev, "area_relativa_pct": frac})


def _m_poly_to_kml_polygon(name: str, poly, to_wgs, color: str):
    # color KML AABBGGRR. Poly simple exterior only for compact export.
    coords = []
    for x, y in list(poly.exterior.coords):
        lon, lat = to_wgs.transform(float(x), float(y))
        coords.append(f"{lon:.8f},{lat:.8f},0")
    return f"""
<Placemark><name>{name}</name><Style><LineStyle><color>{color}</color><width>2</width></LineStyle><PolyStyle><color>33ffffff</color></PolyStyle></Style><Polygon><outerBoundaryIs><LinearRing><coordinates>{' '.join(coords)}</coordinates></LinearRing></outerBoundaryIs></Polygon></Placemark>
"""


def _m_export_morph_kmz(basin_s, basin_h, to_wgs):
    kml = """<?xml version='1.0' encoding='UTF-8'?><kml xmlns='http://www.opengis.net/kml/2.2'><Document><name>HIDROSED_V1 morfometria cuencas</name>"""
    kml += _m_poly_to_kml_polygon("Cuenca soporte", basin_s["polygon"], to_wgs, "ff00aa00")
    kml += _m_poly_to_kml_polygon("Subcuenca hidrologica", basin_h["polygon"], to_wgs, "ff0000ff")
    inter = basin_s["polygon"].intersection(basin_h["polygon"])
    if not inter.is_empty and hasattr(inter, "exterior"):
        kml += _m_poly_to_kml_polygon("Interseccion cuencas", inter, to_wgs, "ff00ffff")
    kml += "</Document></kml>"
    bio = io.BytesIO()
...
def _m_export_morph_excel(morph_s: Dict[str, Any], morph_h: Dict[str, Any], hyp_s: pd.DataFrame, hyp_h: pd.DataFrame):
    bio = io.BytesIO()
    with pd.ExcelWriter(bio, engine="openpyxl") as writer:
        pd.DataFrame([morph_s, morph_h]).to_excel(writer, sheet_name="morfometria", index=False)
        hyp_s.to_excel(writer, sheet_name="hipsometrica_soporte", index=False)
        hyp_h.to_excel(writer, sheet_name="hipsometrica_hidrologica", index=False)
    bio.seek(0)
    return bio.getvalue()


def run_morfometria_cuenca_module():
    st.title("💧 HIDROSED_V1 · Cuenca y propiedades morfométricas")
    st.caption("Este módulo solo aporta propiedades morfométricas de cuenca. El módulo de eje/perfil/secciones se ejecuta separado y replica v15. Versión revisada: compila OK.")
    st.sidebar.header("Morfometría · Entradas")
    pc_s_file = st.sidebar.file_uploader("PC cuenca soporte/general · KMZ/KML", type=["kmz", "kml"], key="m_pc_s")
    pc_h_file = st.sidebar.file_uploader("PC hidrológico/subcuenca · KMZ/KML", type=["kmz", "kml"], key="m_pc_h")
    axis_file = st.sidebar.file_uploader("Eje del cauce · KMZ/KML línea · recomendado", type=["kmz", "kml"], key="m_axis")
    st.sidebar.header("Morfometría · DEM")
    dem_source = st.sidebar.radio("Fuente DEM", ["Descargar desde OpenTopography", "Cargar DEM GeoTIFF manual"], key="m_dem_source")
    dem_file = None
    api_key = ""
    demtype = "COP30"
    margin_km = 25.0
    if dem_source == "Cargar DEM GeoTIFF manual":
        dem_file = st.sidebar.file_uploader("DEM GeoTIFF", type=["tif", "tiff"], key="m_dem_file")
    else:
        demtype = st.sidebar.selectbox("DEM OpenTopography", ["COP30", "NASADEM", "SRTMGL1", "SRTMGL3", "AW3D30"], index=0, key="m_demtype")
        api_key = st.sidebar.text_input("API Key OpenTopography", type="password", key="m_api")
        margin_km = st.sidebar.number_input("Margen descarga DEM alrededor de eje y PCs [km]", min_value=1.0, max_value=120.0, value=25.0, step=1.0, key="m_margin")
    st.sidebar.header("Morfometría · Parámetros")
    resolution = st.sidebar.selectbox("Resolución interna DEM [m]", [30.0, 60.0, 90.0, 120.0], index=1, key="m_res")
    snap_text = st.sidebar.text_input("Radios ajuste al drenaje [m]", value="150,300,600,1000,2000", key="m_snap")
    axis_buffer = st.sidebar.number_input("Buffer eje para ajuste de PC [m]", min_value=50.0, max_value=10000.0, value=1000.0, step=50.0, key="m_axis_buffer")
    run = st.sidebar.button("Procesar morfometría de cuencas", type="primary", key="m_run")

    if run:
        if pc_s_file is None or pc_h_file is None:
            st.error("Debes cargar PC cuenca soporte y PC hidrológico.")
            st.stop()
        if dem_source == "Cargar DEM GeoTIFF manual" and dem_file is None:
            st.error("Debes cargar DEM GeoTIFF manual o usar OpenTopography.")
            st.stop()
        if dem_source == "Descargar desde OpenTopography" and not api_key:
            st.error("Debes ingresar API Key de OpenTopography.")
            st.stop()
        try:
            pc_s_lon, pc_s_lat, _, pc_s_name = pick_first_point(pc_s_file, "PC cuenca soporte")
            pc_h_lon, pc_h_lat, _, pc_h_name = pick_first_point(pc_h_file, "PC hidrológico")
            crs_proj = determine_local_utm_crs(pc_s_lon, pc_s_lat)
            to_proj, to_wgs = transformers_for_crs(crs_proj)
            pc_s = point_projected(pc_s_lon, pc_s_lat, to_proj)
            pc_h = point_projected(pc_h_lon, pc_h_lat, to_proj)
            axis = None
            axis_coords = [(pc_s_lon, pc_s_lat, None), (pc_h_lon, pc_h_lat, None)]
            if axis_file is not None:
                axis_coords, _ = pick_longest_line(axis_file, "eje del cauce")
                axis, _zs = coords_to_linestring_projected(axis_coords, to_proj)
            if dem_source == "Descargar desde OpenTopography":
                west, south, east, north = bbox_from_lonlat_inputs(axis_coords, pc_h_lon, pc_h_lat, pc_s_lon, pc_s_lat, margin_km)
                dem_bytes = download_opentopography_dem(api_key, demtype, west, south, east, north)
                dem_runtime = _m_uploaded_to_bytesupload(f"DEM_{demtype}_morfometria.tif", dem_bytes)
            else:
                dem_runtime = dem_file
            with st.spinner("Procesando DEM, flujo D8, cuencas y propiedades morfométricas..."):
                dem, transform, valid = _m_read_dem_as_projected_array(dem_runtime, crs_proj, target_resolution_m=float(resolution))
                rec = _m_flow_d8(dem, valid)
                acc = _m_flow_accumulation(rec, valid.ravel())
                # Lectura robusta de radios: acepta coma, punto y coma, espacios, tabulaciones y saltos de línea.
                # Se evita depender de re en esta línea para prevenir NameError por caché o despliegues antiguos.
                _snap_clean = str(snap_text).replace(";", ",").replace("\n", ",").replace("\t", ",").replace(" ", ",")
                radii = [float(x.strip()) for x in _snap_clean.split(",") if x.strip()]
                basin_s = _m_delineate_single_basin(pc_s, "Cuenca soporte", dem, transform, valid, rec, acc, radii, axis, axis_buffer)
                basin_h = _m_delineate_single_basin(pc_h, "Subcuenca hidrológica", dem, transform, valid, rec, acc, radii, axis, axis_buffer)
                morph_s = _m_morphometry("Cuenca soporte", basin_s, dem, transform, valid, crs_proj)
                morph_h = _m_morphometry("Subcuenca hidrológica", basin_h, dem, transform, valid, crs_proj)
                hyp_s = _m_hypsometric_curve(dem, transform, valid, basin_s["polygon"])
                hyp_h = _m_hypsometric_curve(dem, transform, valid, basin_h["polygon"])
                result = {"morph_support": morph_s, "morph_hydro": morph_h, "hyp_support": hyp_s, "hyp_hydro": hyp_h, "basin_support": basin_s, "basin_hydro": basin_h, "crs_proj": crs_proj.to_string()}
                st.session_state["hidrosed_morfometria"] = result
            st.success("Morfometría calculada y guardada en sesión para módulos futuros.")
        except Exception as exc:
            st.error("No se pudo procesar la morfometría.")
            st.exception(exc)
            st.stop()

    if "hidrosed_morfometria" not in st.session_state:
        st.info("Carga los puntos de control, DEM y eje recomendado. Luego presiona **Procesar morfometría de cuencas**.")
        return

    result = st.session_state["hidrosed_morfometria"]
    dfm = pd.DataFrame([result["morph_support"], result["morph_hydro"]])
    st.subheader("Propiedades morfométricas")
    st.dataframe(dfm, use_container_width=True)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Área soporte [km²]", f"{result['morph_support']['area_km2']:.2f}")
    c2.metric("Área hidrológica [km²]", f"{result['morph_hydro']['area_km2']:.2f}")
    c3.metric("Pendiente media soporte [%]", f"{result['morph_support']['pendiente_media_pct']:.2f}")
    c4.metric("Confianza hidrológica [%]", f"{result['morph_hydro']['confianza_pct']:.0f}")
    tab_img, tab_a, tab_b, tab_c = st.tabs(["Imagen cuenca delimitada", "Curvas hipsométricas", "Base técnica JSON", "Descargas"])
    with tab_img:
        fig_map, axm = plt.subplots(figsize=(7, 7))
        bs_poly = result["basin_support"]["polygon"]
        bh_poly = result["basin_hydro"]["polygon"]
        inter_poly = bs_poly.intersection(bh_poly)

        def _plot_poly(ax, geom, edgecolor, facecolor, label):
            if geom is None or geom.is_empty:
                return
            ...
            for g in geoms:
                x, y = g.exterior.xy
                ax.fill(x, y, facecolor=facecolor, edgecolor=edgecolor, linewidth=1.8, alpha=0.35, label=label if first else None)
                first = False

        _plot_poly(axm, bs_poly, "#2ca02c", "#98df8a", "Cuenca soporte")
        _plot_poly(axm, bh_poly, "#1f77b4", "#aec7e8", "Subcuenca hidrológica")
        if inter_poly is not None and not inter_poly.is_empty:
            _plot_poly(axm, inter_poly, "#ff7f0e", "#ffbb78", "Intersección")

        try:
            pcsx, pcsy = result["basin_support"]["snapped_point"].x, result["basin_support"]["snapped_point"].y
            pchx, pchy = result["basin_hydro"]["snapped_point"].x, result["basin_hydro"]["snapped_point"].y
            axm.plot(pcsx, pcsy, marker="o", markersize=6, color="#2ca02c", label="PC soporte")
            axm.plot(pchx, pchy, marker="o", markersize=6, color="#1f77b4", label="PC hidrológico")
        except Exception:
            pass

        axm.set_title("Cuenca delimitada y subcuenca hidrológica")
        axm.set_xlabel(f"X proyectado [{result['crs_proj']}]")
        axm.set_ylabel(f"Y proyectado [{result['crs_proj']}]")
    ...
    with tab_a:
        ...
        hs = result["hyp_support"]; hh = result["hyp_hydro"]
        if not hs.empty: ax.plot(hs["area_relativa_pct"], hs["elevacion_m"], label="Cuenca soporte")
        if not hh.empty: ax.plot(hh["area_relativa_pct"], hh["elevacion_m"], label="Subcuenca hidrológica")
        ax.set_xlabel("Área relativa por encima de la cota [%]"); ax.set_ylabel("Elevación [m]"); ax.grid(True); ax.legend()
        st.pyplot(fig)
    with tab_b:
        safe = {"morph_support": result["morph_support"], "morph_hydro": result["morph_hydro"], "crs_proj": result["crs_proj"]}
        st.json(safe)
    with tab_c:
        excel = _m_export_morph_excel(result["morph_support"], result["morph_hydro"], result["hyp_support"], result["hyp_hydro"])
        st.download_button("Descargar morfometría Excel", excel, file_name="HIDROSED_V1_morfometria_cuencas.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        safe_json = json.dumps({"morph_support": result["morph_support"], "morph_hydro": result["morph_hydro"], "crs_proj": result["crs_proj"]}, ensure_ascii=False, indent=2).encode("utf-8")
        st.download_button("Descargar base morfométrica JSON", safe_json, file_name="HIDROSED_V1_morfometria_cuencas.json", mime="application/json")
        kmz = _m_export_morph_kmz(result["basin_support"], result["basin_hydro"], transformers_for_crs(CRS.from_user_input(result["crs_proj"]))[1])
        st.download_button("Descargar KMZ cuencas", kmz, file_name="HIDROSED_V1_cuencas_morfometria.kmz", mime="application/vnd.google-earth.kmz")


def run_base_tecnica_hidrosed():
    st.title("🧩 HIDROSED_V1 · Base técnica para módulos futuros")
    st.markdown("La aplicación conserva en sesión la información calculada por los primeros módulos para conectarla después con hidrología, hidráulica, sedimentos y socavación.")
    morf = st.session_state.get("hidrosed_morfometria")
    eje = st.session_state.get("eje_result")
    data = {
        "cuenca_morfometria_disponible": morf is not None,
        "eje_secciones_disponible": eje is not None,
        "cuenca_morfometria": None if morf is None else {"soporte": morf.get("morph_support"), "hidrologica": morf.get("morph_hydro"), "crs_proj": morf.get("crs_proj")},
        "eje_cauce_secciones": None if eje is None else {
    ...
    st.json(data)
    st.download_button("Descargar base técnica integrada JSON", json.dumps(data, ensure_ascii=False, indent=2).encode("utf-8"), file_name="HIDROSED_V1_base_tecnica_integrada.json", mime="application/json")


def run_eje_cauce_secciones_v15():
    st.title("🌊 HidroSed · Eje Cauce y Secciones v15 · geometría ajustada a extremos")
...
# ======================================================================================
# HIDROSED_V1 · Navegación modular
# ======================================================================================

st.sidebar.title("HIDROSED_V1")
_modulo_hidrosed = st.sidebar.radio(
    "Seleccionar módulo",
    [
        "Cuenca y morfometría",
        "Eje cauce y secciones · v15 idéntico",
        "Base técnica integrada",
    ],
    index=0,
    key="hidrosed_v1_modulo",
)

if _modulo_hidrosed == "Cuenca y morfometría":
    run_morfometria_cuenca_module()
elif _modulo_hidrosed == "Eje cauce y secciones · v15 idéntico":
    run_eje_cauce_secciones_v15()
else:
    run_base_tecnica_hidrosed()
